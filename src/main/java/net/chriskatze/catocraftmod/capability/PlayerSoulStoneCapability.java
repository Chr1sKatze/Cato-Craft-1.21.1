...
import net.chriskatze.catocraftmod.item.SoulStoneItem;
...
/**
 * Stores a single equipped Soul Stone for a player.
 * Handles serialization, syncing, and attribute application.
 * Mirrors PlayerEarringCapability but reserved for archetypes.
 */
public class PlayerSoulStoneCapability extends ItemStackHandler {
    ...
    public PlayerSoulStoneCapability() {
        super(1);
    }
    ...
    public CompoundTag serializeNBT(net.minecraft.core.HolderLookup.Provider registryAccess) {
        ...
        tag.put("Items", list);
        CatocraftMod.LOGGER.debug("[SoulStoneCap] serializeNBT -> {}", tag);
        return tag;
    }
    ...
    public void deserializeNBT(net.minecraft.core.HolderLookup.Provider registryAccess, CompoundTag nbt) {
        ...
        CatocraftMod.LOGGER.debug("[SoulStoneCap] deserializeNBT <- {}", nbt);
    }
    ...
    protected void onContentsChanged(int slot) {
        ...
        dirty = true;
        CatocraftMod.LOGGER.debug("[SoulStoneCap] Slot {} changed, marking dirty", slot);

        if (owner != null && !owner.level().isClientSide) {
            applySoulStoneAttributes();
            SoulStoneSyncHelper.syncToClient(owner);
        }
    }

    // ------------------- Attribute Logic -------------------

    /** Applies modifiers for the currently equipped Soul Stone. */
    private void applySoulStoneAttributes() {
        ...
        // 2️⃣ Apply new modifiers if this is a SoulStoneItem
        if (stack.getItem() instanceof SoulStoneItem soulStone) {
            // placeholder — later we’ll hook in archetype logic
            // (for now it will behave like JewelleryItem)
            var mods = soulStone.getSoulStoneModifiers();
            mods.forEach((holder, modifier) -> {
                ...
                if (instance != null && !instance.hasModifier(modifier.id())) {
                    ...
                    String attrKey = holder.unwrapKey()
                            ...
                            .orElse("[unregistered]");
                    CatocraftMod.LOGGER.debug("[SoulStoneCap] Applied modifier {} to {}", modifier.id(), attrKey);
                }
            });
        }

        // 3️⃣ Normalize health based on attribute changes
